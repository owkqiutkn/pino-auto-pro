...
export default function TransmissionsPage() {
    ...
    const loadTransmissions = useCallback(async () => {
        try {
            ...
            console.error(err);
            setError(getErrorMessage(err, "Failed to load transmissions."));
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleCreate = async (event: FormEvent) => {
        ...
        try {
            ...
            setError("");
            const client = await createSPASassClient();
            const { error: createError } = await client.createTransmission(name.trim());
            if (createError) throw createError;
            setName("");
            await loadTransmissions();
        } catch (err) {
            console.error(err);
            setError(getErrorMessage(err, "Failed to create transmission."));
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            console.error(err);
            setError(getErrorMessage(err, "Failed to delete transmission."));
        }
    };
    ...
    return (
        <div className="space-y-4 max-w-3xl">
            <h1 className="text-2xl font-semibold">Transmissions</h1>
                    ...
                    <tbody>
                        {transmissions.map((transmission) => (
                            <tr key={transmission.id} className="border-t">
                                <td className="p-3">{transmission.name}</td>
                                <td className="p-3">{new Date(transmission.created_at).toLocaleDateString()}</td>
                                <td className="p-3">
                                    <button
